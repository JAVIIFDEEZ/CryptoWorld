...
function cascade(overrides: Partial<RetestCascade> = {}): RetestCascade {
  return {
    survived: true,
    checks: { noise: true, starting_bar: true, skip_trades: true, parameter_sensitivity: true },
    failed: [],
    ...
    note: 'Sobrevive a las cuatro perturbaciones.',
    ...overrides,
  }
}

describe('RetestCascadeCard', () => {
  it('resume que sobrevive a las cuatro pruebas', () => {
    render(<RetestCascadeCard retests={cascade()} />)
    // El veredicto sale en la insignia y de nuevo en la nota del backend.
    expect(screen.getAllByText(/sobrevive a las cuatro/i).length).toBeGreaterThan(0)
  })
    ...
    render(
      <RetestCascadeCard
        retests={cascade({
          ...
          checks: { noise: false, starting_bar: true, skip_trades: false, parameter_sensitivity: true },
        })}
      />,
    )
    expect(screen.getByText(/falla 2 de 4/i)).toBeInTheDocument()
  })
  ...
  it('enumera las cuatro perturbaciones con su pregunta', () => {
    render(<RetestCascadeCard retests={cascade()} />)
